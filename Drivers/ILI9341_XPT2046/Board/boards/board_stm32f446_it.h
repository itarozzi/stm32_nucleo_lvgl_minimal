#ifndef BOARD_STM32F446_H_
#define BOARD_STM32F446_H_

/*
 * STM32F446 board mapping for ILI9341 + XPT2046.
 *
 * All symbols below are generated by STM32CubeMX and appear in main.h / spi.h.
 * GPIO labels (CS, DC, RESET, T_CS, T_IRQ) must match the User Labels you
 * assigned in CubeMX → Pinout & Configuration → GPIO.
 */

/* ── Display (ILI9341 on SPI2) ───────────────────────────────────────────── */
#define BOARD_DISP_SPI_HANDLE        (&hspi2)
#define BOARD_DISP_CS_PORT           TFT_CS_GPIO_Port
#define BOARD_DISP_CS_PIN            TFT_CS_Pin
#define BOARD_DISP_DC_PORT           TFT_DC_GPIO_Port
#define BOARD_DISP_DC_PIN            TFT_DC_Pin
#define BOARD_DISP_RESET_PORT        TFT_RESET_GPIO_Port
#define BOARD_DISP_RESET_PIN         TFT_RESET_Pin
#define BOARD_DISP_HOR_RES           320u   /* logical width after rotation  */
#define BOARD_DISP_VER_RES           240u   /* logical height after rotation */
#define BOARD_DISP_ROTATION          3u     /* 0=portrait  3=landscape       */

/* ── Touch (XPT2046 on SPI3) ─────────────────────────────────────────────── */
#define BOARD_TOUCH_SPI_HANDLE       (&hspi3)
#define BOARD_TOUCH_CS_PORT          T_CS_GPIO_Port
#define BOARD_TOUCH_CS_PIN           T_CS_Pin
#define BOARD_TOUCH_IRQ_PORT         T_IRQ_GPIO_Port
#define BOARD_TOUCH_IRQ_PIN          T_IRQ_Pin

/* ── Touch calibration defaults (raw ADC values at each screen edge) ──────── */
/* Run touch_calibrate() once with printf enabled to obtain these values.      */
#define BOARD_TOUCH_RAW_X_MIN        262u
#define BOARD_TOUCH_RAW_X_MAX        1872u
#define BOARD_TOUCH_RAW_Y_MIN        230u
#define BOARD_TOUCH_RAW_Y_MAX        1872u

#endif /* BOARD_STM32F446_H_ */
