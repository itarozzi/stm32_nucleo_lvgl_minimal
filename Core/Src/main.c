...
int main(void)
{
  ...
  // blink 3 times to notify the Program is started
  for (int i = 0; i < 3; i++)
  {
  ...
  uart2_write("LVGL DISPLAY INIT Ok\r\n");

  TouchController_Init(Board_GetTouchConfig());

  //  load_gui();   // your application UI

  // start the UI generated by eez-studio
  ui_init();

  uart2_write("LVGL UI STARTED\r\n");
  ...
  while (1)
  {
    ...
    if (cnt == 1000) {
      cnt = 0;
      // lv_lock();
      // Assign label created in load_gui (no eez-studio UI)
...
static void btn_event_cb(lv_event_t * e)
{
    if(lv_event_get_code(e) == LV_EVENT_CLICKED) {
      press_counter++;
      lv_label_set_text_fmt(date_label, "Clicked %d", (int)press_counter);
    }
}
